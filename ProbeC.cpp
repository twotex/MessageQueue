...
int main() {
	...
	theExitMessage.mtype = 900; //use mtype 900 for our exit message
	...
	kill_patch(qid, (msgbuf*)&theExitMessage, size, 900); //Execute kill patch immediately so it can be called later when Probe C needs to be terminated
	...
	const int magic_seed = 1129854; //magic seed that will be used in a modulus operation with our random value
	...
	{	
		...
		msg.mtype = 900; //use mtype 900 for messages between Probe C and DataHub
